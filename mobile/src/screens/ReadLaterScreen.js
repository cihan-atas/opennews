...
export default function ReadLaterScreen() {
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`/read-later/?page=${page}&size=${pageSize}`);
      if (res.ok) {
  ...
  const handleRemove = async (newsId) => {
    try {
      const res = await apiFetch(`/read-later/${newsId}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 12 }]}>
      ...
      <View style={styles.header}>
        <Text style={styles.h1}>Sonra Oku</Text>
        <Text style={styles.sub}>{totalCount > 0 ? `${totalCount} haber kuyrukta` : 'Okuma kuyruğun boş.'}</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          ...
          <Text style={{ color: colors.textDim, fontSize: 16, marginTop: 12 }}>Okuma kuyruğun henüz boş.</Text>
        </View>
      ) : (
        <FlatList
          data={items}
